...
public class wheelhinge1manipulator2 : MonoBehaviour
{
    ...
    public void Kinematic(Vector3 position0, Vector3 axis0, float angle0delta, float angle1delta, float angle2delta)
    {
        ...
        transform.RotateAround(transform.position, transform.rotation * Vector3.up, angle1delta + angle2delta);
        GameObject next = GetComponent<FixedJoint>().connectedBody.gameObject;
        next.GetComponent<wheel1manipulator2>().Kinematic(position0, axis0, angle0delta, transform.position, transform.rotation * Vector3.up, angle1delta, angle2delta);
    }
